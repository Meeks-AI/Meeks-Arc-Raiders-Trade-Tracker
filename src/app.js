...
function render() {
  ...
  Object.values(grouped)
    ...
    .forEach((g, i) => {
      ...
      // Auto-fill button: only shown when there's a cached median price
      const fillBtn = mp
  ...
  // Count sessions for divider numbering (newest-first render, so count total first)
...
// Sell entire stack — auto-fills median price if price field is empty
...
function generateRandomHistory() {
  ...
  // Sell same item at a different price for chart interest
...
function renderAnalytics() {
  ...
  Object.entries(stats).sort((a, b) => b[1].profit - a[1].profit).forEach(([name, s]) => {
    ...
    perBody.innerHTML += `<tr>
      <td class="font-semibold" style="cursor:pointer" onclick="showPriceHistory('${safeName}')" title="View price history for ${name}">
  ...
  priceHistoryChart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: 'Median',
          data: Array(prices.length).fill(med),
...
function importData(input) {
  ...
  r.onload = (e) => {
    ...
    if (d.aliasMap) { aliasMap = d.aliasMap; writeJson(STORAGE_KEYS.itemAliases, aliasMap); }
    render();
  };
  r.readAsText(f);
}
